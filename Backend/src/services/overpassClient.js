...
export async function findSafePlaces(lat, lon, radiusM = 1000, bypassCache = false) {
  ...
  console.log(`[Overpass] Running query:\n${query}`);
  ...
  try {
    ...
    console.error("[Overpass] Query error:", err.message);
    if (err.response) {
  ...
  console.log(`[Overpass] Raw elements count returned: ${Array.isArray(elements) ? elements.length : 0}`);
